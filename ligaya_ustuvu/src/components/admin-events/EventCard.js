...
const EventCard = ({ event, onEdit, onDelete }) => {
  ...
  return (
    <>
      <div className={`bg-white rounded-lg shadow-md overflow-hidden ${isPastEvent ? 'border-l-4 border-amber-400' : ''}`}>
        {/* Banner image */}
        <div className="relative w-full h-48">
          {event.bannerImage ? (
            <img 
              ...
              alt={event.title} 
              className={`w-full h-full object-cover ${isPastEvent ? 'opacity-70' : ''}`}
            />
          ) : (
        ...
        <div className="p-4">
          ...
          <div className="flex justify-between items-center">
            {/* Past event indicator for admins */}
            {isPastEvent && (
